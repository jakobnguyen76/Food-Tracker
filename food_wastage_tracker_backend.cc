...
void SerializeFoodWastageRecordToJSON(
  ...
  disposal_mechanism = record.Disposal();
...
FoodWastageRecord DeserializeFoodWastageRecordFromJSON(
  ...
  record.SetReason(json_obj["wastage_reason_"].GetString());
  ...
  record.SetDisposal(json_obj["disposal_mechanism_"].GetString());
...
crow::json::wvalue FoodWastageReportToCrowJSON(
  ...
  most_common_disposal_mechanisms = report.CommonMechanisms();
  ...
  most_commonly_wasted_foods = report.CommonWasted();
  ...
  most_common_wastage_reasons = report.CommonReasons();
  ...
  most_costly_waste_producing_meals = report.Costly();
  ...
  suggested_strategies_to_reduce_waste = report.Strategies();
...
FoodWastageRecord QueryStringToFoodWastageRecord(
  ...
  record.SetReason(query_string.get("wastage_reason"));
...
bool FoodWastageTrackerBackend::LoadRecordsFromJSONFile() {
  ...
  for (rapidjson::Value::ConstValueIterator itr = doc.Begin(); itr != doc.End();
    ...
    food_wastage_tracker_.AddFoodWastageRecord(record);
  }
...
bool FoodWastageTrackerBackend::WriteRecordsToJSONFile() const {
  ...
  // TODO: Call the member function in the FoodWastageTracker class, on the
  // member object that you added in food_wastage_tracker_backend.h, that
  // returns all the FoodWastageRecord objects. Store the returned records in
  // the vector declared above. Also change the data type of the records vector
  // to `const std::vector<FoodWastageRecord>&`.

  records = food_wastage_tracker_.GetTracker();
...
crow::json::wvalue FoodWastageTrackerBackend::AddRecord(
  ...
  add_result = food_wastage_tracker_.AddFoodWastageRecord(record);
...
crow::json::wvalue FoodWastageTrackerBackend::DeleteRecord(
  ...
  // TODO: Call the member function in the FoodWastageTracker class, on the
  // member object that you added in food_wastage_tracker_backend.h, that
  // deletes `record` and returns the status of the delete operation as a bool.
  // Store the returned value in the bool declared above.

  delete_result = food_wastage_tracker_.DeleteFoodWastageRecord(record);
...
crow::json::wvalue FoodWastageTrackerBackend::GetFoodWastageReport() const {
  ...
  // TODO: Call the member function in the FoodWastageTracker class, on the
  // member object that you added in food_wastage_tracker_backend.h, that
  // generates a FoodWastageReport object using all the FoodWastageRecords and
  // returns it. Store the returned value in the `generated_report` object
  // declared above.
  std::cout << "get trial\n";
  generated_report = food_wastage_tracker_.GetTracker();
  std::cout << "get trial complete\n";
  return FoodWastageReportToCrowJSON(generated_report);
  std::cout << "returned report\n";
}
